'use client';

import JobCard from './JobCard';

const JobListing = ({ jobs = [], loading = false, error = null }) => {
  if (loading) {
    return (
      <div className="space-y-1">
        {/* Compact loading skeleton */}
        {[...Array(8)].map((_, index) => (
          <div key={index} className="border-b border-gray-700/50 p-3">
            <div className="flex items-center gap-3 animate-pulse">
              <div className="w-10 h-10 bg-gray-700 rounded"></div>
              <div className="flex-1 space-y-2">
                <div className="h-4 bg-gray-700 rounded w-3/4"></div>
                <div className="h-3 bg-gray-700 rounded w-1/2"></div>
                <div className="h-3 bg-gray-700 rounded w-2/3"></div>
              </div>
              <div className="h-4 bg-gray-700 rounded w-20"></div>
            </div>
          </div>
        ))}
      </div>
    );
  }

  if (error) {
    return (
      <div className="text-center py-8 px-4 border border-red-500/20 rounded bg-red-500/5">
        <div className="text-red-400 mb-2">
          <svg className="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          <h3 className="text-white font-semibold">Error loading jobs</h3>
          <p className="text-red-300 text-sm mt-1">{error}</p>
        </div>
      </div>
    );
  }

  if (!jobs || jobs.length === 0) {
    return (
      <div className="text-center py-8 px-4">
        <div className="text-gray-500 mb-2">
          <svg className="w-8 h-8 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <h3 className="text-white font-semibold">No jobs found</h3>
          <p className="text-gray-400 text-sm mt-1">Try adjusting your search criteria.</p>
        </div>
      </div>
    );
  }

  return (
    <div className="bg-gray-800/30 rounded border border-gray-700/50">
      {jobs.map((job, index) => (
        <JobCard 
          key={`job-${index}`} 
          job={job} 
        />
      ))}
    </div>
  );
};

export default JobListing;
